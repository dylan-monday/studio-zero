...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'PATCH') {
    ...
    if (action === 'cancel') {
      ...
      // Send appropriate email based on previous booking status
      const guest = booking.guest;
      if (guest) {
        const wasConfirmed = booking.status === 'confirmed' || booking.status === 'approved';
        if (wasConfirmed) {
          await sendEmail(guest.email, 'Reservation Cancelled — Studio Zero SF', cancellationEmailHtml(guest.first_name, booking));
        } else {
...
function cancellationEmailHtml(guestName: string, booking: any): string {
  const checkIn = new Date(booking.check_in).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  const checkOut = new Date(booking.check_out).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  ...
  return `
    <div style="font-family: 'DM Sans', system-ui, -apple-system, sans-serif; max-width: 560px; margin: 0 auto; padding: 40px 20px; background: #faf9f7;">
      ...
      <div style="margin-bottom: 32px;">
        <p style="font-family: monospace; font-size: 11px; text-transform: uppercase; letter-spacing: 0.2em; color: #78716c; margin: 0 0 8px 0;">Cancelled</p>
        <h1 style="font-family: Georgia, 'Times New Roman', serif; font-size: 28px; font-weight: 400; color: #1c1917; margin: 0; letter-spacing: -0.01em;">Hi ${guestName},</h1>
      </div>
      <p style="color: #78716c; font-size: 15px; line-height: 1.65; margin: 0 0 28px 0;">
        Your reservation for ${checkIn} through ${checkOut} has been cancelled. Your payment of $${booking.total_amount.toFixed(2)} has been fully refunded — you should see it back in your account within 5–10 business days, depending on your bank.
      </p>
      <p style="color: #78716c; font-size: 15px; line-height: 1.65; margin: 0 0 28px 0;">
        If your dates are flexible, feel free to check availability and try again.
      </p>
      <div style="text-align: center; margin-bottom: 32px;">
        <a href="https://studiozerosf.com/book" style="display: inline-block; background: #1c1917; color: #faf9f7; padding: 14px 32px; text-decoration: none; font-size: 14px; font-weight: 500; letter-spacing: 0.02em;">Check Availability</a>
      </div>
